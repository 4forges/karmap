  ...
  class ThreadPool
    ...
    def all
      Thread.list.select do |t|
        (t[:internal_key] || '').starts_with?(Karma::Thread::ManagedThread.internal_key_prefix)
      end
    end
    ...
    def active
      @list.select do |managed_thread|
        managed_thread.initing? || managed_thread.running?
      end
    end

    def stopping
      @list.select(&:stopping?)
    end
    ...
    def get_first_thread_index
      ...
      range = (0..)
      range.lazy.find { |x| !running_indexes.include?(x) }
    end
